...
<div class="saved-page">
  ...
  <div class="saved-header">
    <div>
      <h1>Saved Items</h1>
      <p>Items you've bookmarked to keep an eye on.</p>
    </div>
    ...
    <div class="saved-grid">
      <?php foreach ($saved_items as $item): ?>
        <div class="saved-card <?= $item['status'] !== 'active' ? 'saved-card-unavailable' : '' ?>">
          ...
          <!-- Actions: View + Remove, always equal width -->
          <div class="saved-card-actions">
            <a href="listing.php?id=<?= (int)$item['item_id'] ?>"
               class="saved-btn-view <?= $item['status'] !== 'active' ? 'saved-btn-view-disabled' : '' ?>">
              View
            </a>
            <form method="POST">
              <input type="hidden" name="item_id" value="<?= (int)$item['item_id'] ?>">
              <button type="submit" name="unsave_item" class="saved-btn-remove"
                onclick="return confirm('Remove this item from your saved list?')">
                Remove
              </button>
            </form>
          </div>
